...
const Chat = () => {
    ...
    return (
        <div className="flex flex-row w-full gap-4 p-4">
            {/* Sidebar with Groups and Users */}
            <div className="p-6 bg-white w-1/2 rounded-lg">
                <ul className="space-y-4 mb-4">
                    {groups.length > 0 ? groups.map(group => (
                        <li key={group.id} className="p-4 bg-gray-50 hover:bg-gray-100 rounded-lg shadow-md cursor-pointer"
                            onClick={() => handleGroupDisplay(group.name, group.id)}>
                ...
                <ul className="space-y-4">
                    {users.length > 0 ? users.map(user => (
                        <li key={user.id} className="p-4 bg-gray-50 hover:bg-gray-100 rounded-lg shadow-md cursor-pointer"
                            onClick={() => handleChatDisplay(user.name, user.id)}>
                ...
                <div className="p-6 bg-gray-50 w-1/2 rounded-lg flex flex-col">
                    ...
                    <div className="mt-4">
                        <textarea value={message} onChange={(e) => setMessage(e.target.value)}
                            className="w-full p-2 border rounded-lg text-black" placeholder="Type a message"></textarea>
